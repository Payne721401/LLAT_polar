...
def transfer_FCNV2_DLAMPty_with_radius(
    ...
    DLAMP_info,
    radius=7.5,
    polar_bdy_mask=False,
    polar_bdy_mask_radius=8):
    ...
    if polar_bdy_mask:
        ...
        dis_grid = dis_grid.reshape([-1])
        grid_mask = np.ones(xx.shape) 
        grid_mask[dis_grid<polar_bdy_mask_radius] = np.nan
        polar_bdy_mask_xx = xx[~np.isnan(grid_mask)]
        polar_bdy_mask_yy = yy[~np.isnan(grid_mask)]
    ...
    if polar_bdy_mask:
        FCNV2_bdy_index_xx = polar_bdy_mask_xx + np.int_((90-TC_center_lat)/0.25)-40
        FCNV2_bdy_index_yy = polar_bdy_mask_yy + np.int_((TC_center_lon)/0.25)-40
        for var in sfc_replace_variables:
            DLAMPty_input_surface[polar_bdy_mask_xx,polar_bdy_mask_yy,DLAMP_info['surface_vars'].index(var)] = FCNV2_input[FCNV2_sfc_vars.index(var), FCNV2_bdy_index_xx,FCNV2_bdy_index_yy]
        
        for var in upper_replace_variables:
            DLAMPty_input_upper[:, polar_bdy_mask_xx,polar_bdy_mask_yy,DLAMP_info['upper_vars'].index(var)] = FCNV2_input[(FCNV2_upper_vars.index(var)*13+8):(FCNV2_upper_vars.index(var)*13+21) , 
                    FCNV2_bdy_index_xx, FCNV2_bdy_index_yy]
            
        FCNV2_q = rh_to_q(FCNV2_input[60:73, lat_index, :][:, :,lon_index],FCNV2_data[47:60, lat_index, :][:, :,lon_index],pressure_levels)
        DLAMPty_input_upper[:, polar_bdy_mask_xx,polar_bdy_mask_yy, 3] =FCNV2_q[:, polar_bdy_mask_xx,polar_bdy_mask_yy]
