...
Start with `--mode one-way --max-starts 1`: one initial time, no feedback into
the global model, which is the smallest thing that produces a plottable forecast.
...
def main(args):
    ...
    for initial_time in starts:
        ...
        fcnv2_ic = os.path.join(args.data_root, args.tc_id, 'ERA5', 'for_FCNV2',
                                f"analysis_{stamp}.npy")
        llat_ic = os.path.join(args.data_root, args.tc_id, 'ERA5', 'for_DLAMPty',
                               f"{args.tc_id}_{stamp}_combined.nc")
        for p in (fcnv2_ic, llat_ic):
        ...
        for i in range(1, args.hours // 6 + 1):
            ...
            # lateral boundary from FCNV2, but the global model is untouched.
...
if __name__ == "__main__":
    ...
    p.add_argument("--coupling-root", required=True,
                   help="checkout of the coupling repo (FCNV2 + interaction_tools)")
    p.add_argument("--fcnv2-weight", required=True,
                   help="directory holding weights.tar, global_means.npy, global_stds.npy")
    ...
    p.add_argument("--mode", choices=["one-way", "two-way"], default="two-way")
