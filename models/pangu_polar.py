...
class PanguPolarModel(nn.Module):
    """
    ...
    def __init__(
        ...
        embed_dim: int,
        patch_shape: Shape_T,
        window_size1: Shape_T,
        window_size2: Shape_T,
        ...
        Args:
            ...
            window_size1 (tuple[int, int, int]): window size for layer1 and layer4.
            window_size2 (tuple[int, int, int]): window size for layer2 and layer3.
...
class EarthSpecificBlock(nn.Module):
    def __init__(
        ...
        if roll:
            ...
            # θ (W 軸) 是週期維:整條當單一區段,允許跨縫 (0°/360°) 注意 —— 移除經度式的牆
            w_slices = (slice(None),)
            cnt = 0
            for z in z_slices:
                for h in h_slices:
...
def GetPad3D(img_shape: tuple[int, int, int],
    ...
    Returns:
        function that pads tensor (N,C,Z,H,W)
    """

    Z, H, W = img_shape
    z, h, w = sub_shape

    pz = (floor((-Z % z)/2), ceil((-Z % z)/2))
    ph = (floor((-H % h)/2), ceil((-H % h)/2))
    pw = (floor((-W % w)/2), ceil((-W % w)/2))

    def pad_fn(x: torch.Tensor):
        # x: (N,C,Z,H,W)
        if pw != (0,0): x = F.pad(x, (*pw, 0,0, 0,0), mode="circular")          # W
        if ph != (0,0): x = F.pad(x, (0,0, *ph, 0,0), mode="replicate")         # H (was reflect)
        if pz != (0,0): x = F.pad(x, (0,0, 0,0, *pz), mode="constant", value=0) # Z
        return x

    return pad_fn

def GetPad2D(img_shape: tuple[int, int],
    ...
    def pad_fn(x: torch.Tensor):
        ...
        if pw != (0,0):
            x = F.pad(x, (*pw, 0,0), mode='circular')  # W

        if ph != (0,0):
            x = F.pad(x, (0,0, *ph), mode='replicate')   # H
