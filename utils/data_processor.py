...
def latlon_to_polar(
    ...
    Theta: int = 180,                  # degree bins
    r_max: float = 40.0,               # max radius in grid units (pixels)
    center_xy: tuple[float, float] = (40.0, 40.0),  # (cx in lon/X, cy in lat/Y)
    mode: str = "bilinear",            # "bilinear" or "nearest"
    padding_mode: str = "zeros",       # "zeros" or "border"
    align_corners: bool = True,
    return_theta_deg: bool = True,
):
    """
    Accepts (no batch dim):
      4D: (A, Y=lat, X=lon, V) -> (A, Theta, R, V)
      3D: (Y=lat, X=lon, V)    -> (Theta, R, V)
    ...
    if data.ndim == 4:
        ...
        A = 1
        has_altitude = False
        data = data.unsqueeze(0)  # -> (1,Y,X,V) treat as A=1
    else:
    ...
    inp = data.permute(0, 3, 1, 2).contiguous()           # (A,V,Y,X)
    inp = inp.view(1, A * V, Y, X)                        # (1,C,Y,X)
    ...
    # unpack to (A, Theta, R, V)
    out = out.view(A, V, R, Theta).permute(0, 2, 3, 1).contiguous()  # (A,Theta,R,V)
