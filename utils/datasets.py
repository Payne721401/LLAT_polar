...
from utils.data_processor import combine_ncs_to_xarray, latlon_to_polar
...
class ERA5TCDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 r_degree_max: int,
                 original_resolution: float = 0.25,
    ...
    def __getitem__(self, index: int) -> tuple[WeatherData, WeatherData] | tuple[WeatherData, WeatherData, Stat_T] | tuple[WeatherData, WeatherData, Stat_T, str]:
        ...
        if self.plugin_additional_vars:
            ...
            if not torch.isfinite(stacked_input[0]).all():
                bad = torch.nonzero(~torch.isfinite(stacked_input[0]))[0].tolist()
                with open("log.txt", "a") as f:
                    f.write(upper_path_input)
                    f.write(f"[NaN/Inf] upper bad_idx={bad}\n")
            
            if not torch.isfinite(stacked_input[1]).all():
                bad = torch.nonzero(~torch.isfinite(stacked_input[1]))[0].tolist()
                with open("log.txt", "a") as f:
                    f.write(surface_path_input)
                    f.write(f"[NaN/Inf] surface bad_idx={bad}\n")
            ...
            if not torch.isfinite(stacked_target[0]).all():
                bad = torch.nonzero(~torch.isfinite(stacked_target[0]))[0].tolist()
                with open("log.txt", "a") as f:
                    f.write(upper_path_target)
                    f.write(f"[NaN/Inf] upper bad_idx={bad}\n")
            if not torch.isfinite(stacked_target[1]).all():
                bad = torch.nonzero(~torch.isfinite(stacked_target[1]))[0].tolist()
                with open("log.txt", "a") as f:
                    f.write(surface_path_target)
                    f.write(f"[NaN/Inf] surface bad_idx={bad}\n")
